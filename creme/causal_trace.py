...
def locate_toxic_layer(model, tokenizer, ori_prompt, pert_prompt, layers):
    ...
    input = tokenizer([ori_prompt, pert_prompt], return_tensors="pt", padding=True, truncation=True).to(device)
    with torch.no_grad():
        outputs = model(**input, output_hidden_states=True)
    hidden_states = outputs.hidden_states  # List of (batch_size=2, seq_len, hidden_dim)
    max_layer = None
    max_l2 = float('-inf')
    for layer in layers:
...
def L2_causal_trace(mt,task_id,dic_path,pert_type,ori_problem,pert_problem,batch_size=5, num_iterations=1):
    ...
    acc_orig, passk1 = evaluate_prompt(mt.model, mt.tok, orig_prompt, problem, batch_size, num_iterations)
    acc_pert, passk2 = evaluate_prompt(mt.model, mt.tok, pert_prompt, problem, batch_size, num_iterations)
    print(f"original pass@1: {acc_orig:.2f} | perturbed pass@1: {acc_pert:.2f}")
    write_csv_header_if_not_exists(summary_csv, ["task_id", "layer", "pass@1", "pass@5", "pass@10", "pass_ratio","improvement"])
    append_row_to_csv(summary_csv, [problem["task_id"],"orig", *passk1, acc_orig,None])
    append_row_to_csv(summary_csv, [problem["task_id"],"pert", *passk2, acc_pert,None])
    print("\n====================")
    ...
    for layer in range(mt.num_layers):
        ...
        with torch.no_grad(), nethook.TraceDict(
            ...
            pass_count = 0
            for i in range(1, batch_size + 1):
                completion = mt.tok.decode(out[i][input_ids_cutoff:], skip_special_tokens=True)
                code = filter_code(fix_indents(completion))
                result = check_correctness(problem, code, timeout=3.0, completion_id=i)
                if result["passed"]:
                    pass_count += 1
            ...
            append_row_to_csv(summary_csv, [problem["task_id"], layer, *passk, ratio,improvement])
    if best_restore==0:
        best_layer=best_pass_rate_layer
    print(f"\n✅ Completed intervention, the results are saved to{summary_csv} and {code_json}")
    print(f"✅ The key layer is layer {best_layer}; Restoration Improvement:{best_restore:.2f}")
    key_layer=locate_toxic_layer(mt.model, mt.tok, orig_prompt, pert_prompt, best_layer)
    return key_layer
...
def mbpp_L2_causal_trace(mt,task_id,dic_path,pert_type,ori_problem,pert_problem,batch_size=5, num_iterations=1):
    ...
    acc_orig, passk1 = evaluate_mbpp_prompt(mt.model, mt.tok, orig_prompt, problem, batch_size, num_iterations)
    acc_pert, passk2 = evaluate_mbpp_prompt(mt.model, mt.tok, pert_prompt, pert_problem, batch_size, num_iterations)
    print(f"original pass@1: {acc_orig:.2f} | perturbed pass@1: {acc_pert:.2f}")
    write_csv_header_if_not_exists(summary_csv, ["task_id", "layer", "pass@1", "pass@5", "pass@10", "pass_ratio","improvement"])
    append_row_to_csv(summary_csv, [problem["task_id"],"orig", *passk1, acc_orig,None])
    append_row_to_csv(summary_csv, [problem["task_id"],"pert", *passk2, acc_pert,None])
    ...
    input_batch = tokenizer(
        [orig_prompt.replace("    ", "\t")] + [pert_prompt.replace("    ", "\t")] * batch_size,
        return_tensors="pt",
        padding=True,
        truncation=True,
    ).to(mt.model.device)
    ...
    for layer in range(mt.num_layers):
        ...
        with torch.no_grad(), nethook.TraceDict(
            ...
            for i in range(1, batch_size + 1):
                completion = mt.tok.decode(out[i][input_ids_cutoff:], skip_special_tokens=True)
                code = filter_code(fix_indents(completion))
                result = check_correctness_mbpp(orig_prompt,problem, code, timeout=3.0, completion_id=i)
                if result["passed"]:
                    pass_count += 1
                append_json_record(code_json, {
                    "task_id": problem["task_id"],
    ...
    if best_restore==0:
        best_layer=best_pass_rate_layer
    print(f"\n✅ Completed intervention, the results are saved to {summary_csv} and {code_json}")
    print(f"✅ he key layer is layer {best_layer}; Restoration Improvement: {best_restore:.2f}")
    key_layer=locate_toxic_layer(mt.model, mt.tok, orig_prompt, pert_prompt, best_layer)
    return key_layer
