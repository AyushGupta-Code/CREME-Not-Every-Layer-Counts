...
def model_editing(pert_type, task_name):
    ...
    for task in task_list:
        ...
        if "humaneval" in task_name:
            ...
            ori_problem = get_problem(
                task_id, "data/humaneval/original/HumanEval.jsonl")
            pert_problem = get_problem(
                task_id, f"data/humaneval/perturbed/{pert_type}.jsonl")
            orig_prompt = ori_problem["prompt"].replace("    ", "\t")
            pert_prompt = pert_problem["prompt"].replace("    ", "\t")
            if len(editor.hparams.layers) == 0:
                key_layer = L2_causal_trace(
                    editor, task, dic_path, pert_type, ori_problem, pert_problem, batch_size=10, num_iterations=1)
            ...
            acc_edit, passk3 = evaluate_prompt(
                edited_model, editor.tok, pert_problem["prompt"], ori_problem)
            print("edited_result:", acc_edit, passk3)
            append_row_to_csv(
                summary_csv, [task_id, "edit", task_id, *passk3, acc_edit])
            for task2 in test_list:
                if task2 == task:
                    continue
                task_id2 = f"HumanEval/{task2}"
                pert_problem2 = get_problem(
                    task_id2, f"data/humaneval/perturbed/{pert_type}.jsonl")
                acc_edit2, passk4 = evaluate_prompt(
                    edited_model, editor.tok, pert_problem2["prompt"], pert_problem2)
                append_row_to_csv(
                    summary_csv, [task_id2, "edit", task_id, *passk4, acc_edit2])
                print(f"task{task2} result after edit:", acc_edit2, passk4)
        elif "mbpp" in task_name:
            task_id = task
            ori_problem = get_mbpp_problem(
                task_id, "data/mbpp/original/mbpp_original.jsonl")
            pert_problem = get_mbpp_problem(
                task_id, f"data/mbpp/perturbed/{pert_type}.jsonl")
            ...
            if len(editor.hparams.layers) == 0:
                key_layer = mbpp_L2_causal_trace(
                    editor, task, dic_path, pert_type, ori_problem, pert_problem, batch_size=10, num_iterations=1)
                editor.hparams.layers = [key_layer]
                print("find key layer:", key_layer)
            edited_model, weights_copy = apply_my_knowledge_edit_to_model(
            ...
            acc_edit, passk3 = evaluate_mbpp_prompt(
                edited_model, editor.tok, pert_prompt, ori_problem)
            print("edited_result:", acc_edit, passk3)
            append_row_to_csv(
                summary_csv, [task_id, "edit", task_id, *passk3, acc_edit])
            for task2 in test_list:
                if task2 == task:
                    continue
                task_id2 = task2
                pert_problem2 = get_mbpp_problem(
                    task_id2, f"data/mbpp/perturbed/{pert_type}.jsonl")
                pert_prompt2 = build_prompt(pert_problem2)
                acc_edit2, passk4 = evaluate_mbpp_prompt(
                    edited_model, editor.tok, pert_prompt2, pert_problem2)
                append_row_to_csv(
                    summary_csv, [task_id2, "edit", task_id, *passk4, acc_edit2])
                print(f"task{task2} result after edit:", acc_edit2, passk4)
        del editor
